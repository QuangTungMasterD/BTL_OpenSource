...
$search = $_GET['s'] ?? '';
$totalPage = getTotalPageUnit(htmlspecialchars($search), ($_SESSION['role'] == 'Admin' ? '' : $_SESSION['user_id']));
...
$units = handleGetUnitByPage(htmlspecialchars($search), $page, ($_SESSION['role'] == 'Admin' ? '' : $_SESSION['user_id']));
$total = count($units);
$totalUnit = count(handleGetUnitByPage('', $totalPage > 0 ? $totalPage : 1, ($_SESSION['role'] == 'Admin' ? '' : $_SESSION['user_id']))) + 40 * (($totalPage - 1 >= 0 ? ($totalPage - 1) : 0));
...
<head>
  ...
  <script src="./../../../js/manager/units/index.js"></script>
  ...
  <div class="manager grid grid-cols-5 gap-3 mt-[var(--height-header)] ">
    ...
    <div class="manager-container col-span-4 ">
      <div class="manager-content pt-3">
        <div class="text-[40px] tracking-wide font-bold font-sans mb-2">Danh sách chương học</div>
        <div class="group-content w-[calc(100%-12px)]">
          <div class="action mt-1 mb-6 flex">
            <div class="flex flex-1">
              <a href="./add.php" class="btn primary flex items-center"><i class="fa-solid fa-plus"></i> Thêm chương học</a>
            <form class="relative" action="./../../../handle/unit_handle.php" method="GET">
            ...
            <div class=""><p class="btn outline flex items-center !cursor-default"><?=$totalUnit?> chương</p></div>
          </div>
            ...
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
              <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                  <? if ($total == 0) { ?>
                    <tr>
                      <td colspan="5" class="text-center py-4 text-gray-500">Không tìm thấy chương nào.</td>
                    </tr>
                  <? }; ?>
                  ...
                  ?>
                      <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="px-6 py-4">
                          <?= htmlspecialchars($units[$i]['nameUnit']) ?>
                        </td>
                        <td class="px-6 py-4">
                          <?= htmlspecialchars($units[$i]['nameCourse']) ?>
                        </td>
                        <td class="px-6 py-4">
                          <?= htmlspecialchars($units[$i]['username']) ?>
                        </td>
                        <td class="px-6 py-4">
                          <?= htmlspecialchars($units[$i]['createAt']) ?>
                        </td>
                        <td class="px-6 py-4 text-right action-object">
                          <a href="./edit.php?id=<?= $units[$i]['idUnit'] ?>" class=""><i class="fa-solid fa-pen-to-square action-object-item"></i></a>
                          <!-- <a href="#" class=""><i class="fa-solid fa-circle-info action-object-item"></i></a> -->
                          <button
                            data-modal-target="popup-modal"
                            data-modal-toggle="popup-modal"
                            data-unit-id="<?= htmlspecialchars($units[$i]['idUnit']) ?>"
