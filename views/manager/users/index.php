...
checkLogin();
isAdminLogin();
require '../../../handle/user_handle.php';

$search = $_GET['s'] ?? '';
$totalPage = getTotalPageUser(htmlspecialchars($search));
...
$users = handleGetUser(htmlspecialchars($search), $page);
...
<head>
  ...
  <link rel="stylesheet" href="http://localhost/BTL-N2/css/manager/index.css">
  <link rel="stylesheet" href="http://localhost/BTL-N2/fontawesome-free-7.1.0-web/css/all.min.css">
  <link rel="stylesheet" href="http://localhost/BTL-N2/css/manager/students/index.css">

  <link href="https://cdn.jsdelivr.net/npm/flowbite@3.1.2/dist/flowbite.min.css" rel="stylesheet" />
  <script src="https://cdn.jsdelivr.net/npm/@tailwindcss/browser@4"></script>
  <script src="https://cdn.jsdelivr.net/npm/flowbite@3.1.2/dist/flowbite.min.js"></script>
  <script src="./../../../js/manager/users/index.js"></script>
  ...
  <div class="manager grid grid-cols-5 gap-3 mt-[var(--height-header)] ">
    ...
    <div class="manager-container col-span-4 ">
      <div class="manager-content pt-3">
        <div class="text-[40px] tracking-wide font-bold font-sans mb-2">Danh sách người dùng</div>
        <div class="group-content w-[calc(100%-12px)]">
          <div class="action mt-1 mb-6 flex">
            <div class="flex flex-1">
              <a href="./add.php" class="btn primary flex items-center"><i class="fa-solid fa-plus"></i> Thêm người dùng</a>
              <a href="./add-file.php" class="btn primary flex items-center"><i class="fa-solid fa-plus"></i> Nhập file</a>
              <form class="relative" action="./../../../handle/user_handle.php" method="GET">
                <input type="hidden" name="action" value="search">
                <input name="s" value="<?php echo isset($_GET['s']) ? htmlspecialchars($_GET['s']) : ''; ?>" type="text" class="input-search" placeholder="Tìm kiếm người dùng">
            ...
            <div class=""><p class="btn outline flex items-center !cursor-default"><?=$totalUser?> người dùng</p></div>
          </div>
            ...
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
              <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                  <? if ($total == 0) { ?>
                    <tr>
                      <td colspan="5" class="text-center py-4 text-gray-500">Không tìm thấy người dùng nào.</td>
                    </tr>
                  <? }; ?>
                  ...
                  $index = 1;
                  for ($i = 0; $i < $total; $i++) {
                    if($users[$i]['nameRole'] == 'Admin') continue;
                    if ($i < $total - 1) {
                  ?>
                      <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="px-6 py-4">
                          <?= htmlspecialchars($users[$i]['username']) ?>
                        </td>
                        <td class="px-6 py-4">
                          <?= htmlspecialchars($users[$i]['phone']) ?>
                        </td>
                        <td class="px-6 py-4">
                          <?if($users[$i]['nameRole'] == 'Teacher') {?>
                            <p class="roleTeacher"><?= htmlspecialchars($users[$i]['nameRole']) ?></p>
                          <?} else if($users[$i]['nameRole'] == 'Student') {?>
                            <p class="roleStudent"><?= htmlspecialchars($users[$i]['nameRole']) ?></p>
                          <?}?>
                        </td>
                        <td class="px-6 py-4 text-right action-object">
                          <a href="./edit.php?id=<?=$users[$i]['idUser']?>" class=""><i class="fa-solid fa-pen-to-square action-object-item"></i></a>
                          <!-- <a href="#" class=""><i class="fa-solid fa-circle-info action-object-item"></i></a> -->
                          <button
                            data-modal-target="popup-modal"
                            data-modal-toggle="popup-modal"
                            data-user-id="<?= htmlspecialchars($users[$i]['idUser']) ?>"
                    ...
                    ?>
                      <tr class="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="px-6 py-4">
                          <?= htmlspecialchars($users[$total - 1]['username']) ?>
                        </td>
                        <td class="px-6 py-4">
                          <?= htmlspecialchars($users[$total - 1]['phone']) ?>
                        </td>
                        <td class="px-6 py-4">
                          <?if($users[$i]['nameRole'] == 'Teacher') {?>
                            <p class="roleTeacher"><?= htmlspecialchars($users[$i]['nameRole']) ?></p>
