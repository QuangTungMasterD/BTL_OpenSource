...
$search = $_GET['s'] ?? '';
$totalPage = getTotalPageRating(htmlspecialchars($search), ($_SESSION['role'] == 'Admin' ? '' : $_SESSION['user_id']));
...
$ratings = handleGetRatingByPage(htmlspecialchars($search), $page, ($_SESSION['role'] == 'Admin' ? '' : $_SESSION['user_id']));
$total = count($ratings);

$totalRating = count(handleGetRatingByPage('', $totalPage > 0 ? $totalPage : 1, ($_SESSION['role'] == 'Admin' ? '' : $_SESSION['user_id']))) + 40 * (($totalPage - 1 >= 0 ? ($totalPage - 1) : 0));
...
<head>
  ...
  <script src="./../../../js/manager/ratings/index.js"></script>
  ...
  <div class="manager grid grid-cols-5 gap-3 mt-[var(--height-header)] ">
    ...
    <div class="manager-container col-span-4 ">
      <div class="manager-content pt-3">
        <div class="text-[40px] tracking-wide font-bold font-sans mb-2">Danh sách đánh giá</div>
        <div class="group-content w-[calc(100%-12px)]">
          <div class="action mt-1 mb-6 flex">
            <div class="flex flex-1">
              <?if($_SESSION['role'] == 'Admin') {?><a href="./add.php" class="btn primary flex items-center"><i class="fa-solid fa-plus"></i> Thêm đánh giá</a><?}?>
              <form class="relative" action="./../../../handle/rating_handle.php" method="GET">
                <input type="hidden" name="action" value="search">
                <input name="s" value="<?php echo isset($_GET['s']) ? htmlspecialchars($_GET['s']) : ''; ?>" type="text" class="input-search" placeholder="Tìm kiếm đánh giá">
            ...
            <div class=""><p class="btn outline flex items-center !cursor-default"><?=$totalRating?> đánh giá</p></div>
          </div>
                ...
                <tbody>
                  ...
                  ?>
                      <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="px-6 py-4">
                          <?= htmlspecialchars($ratings[$i]['nameCourse']) ?>
                        </td>
                        <td class="px-6 py-4">
                          <?= htmlspecialchars($ratings[$i]['username']) ?>
                        </td>
                        <td class="px-6 py-4">
                          <?= htmlspecialchars($ratings[$i]['rated']) ?>
                        </td>
                        <?if($_SESSION['role'] == 'Admin') {?>
                        <td class="px-6 py-4 text-right action-object">
                          <a href="./edit.php?id=<?= $ratings[$i]['idRating'] ?>" class=""><i class="fa-solid fa-pen-to-square action-object-item"></i></a>
                          <!-- <a href="#" class=""><i class="fa-solid fa-circle-info action-object-item"></i></a> -->
                          <button
                            data-modal-target="popup-modal"
                            data-modal-toggle="popup-modal"
                            data-rating-id="<?= htmlspecialchars($ratings[$i]['idRating']) ?>"
