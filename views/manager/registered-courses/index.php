...
$search = $_GET['s'] ?? '';
$totalPage = getTotalPageRegisteredCourse(htmlspecialchars($search));
...
$registeredCourses = handleGetRegisteredCourseByPage(htmlspecialchars($search), $page);
$total = count($registeredCourses);

$totalRegiateredCourse = count(handleGetRegisteredCourseByPage('', $totalPage > 0 ? $totalPage : 1)) + 40 * (($totalPage - 1 >= 0 ? ($totalPage - 1) : 0));
...
<head>
  ...
  <link rel="stylesheet" href="http://localhost/BTL-N2/css/manager/registered-courses/index.css">
  ...
  <script src="./../../../js/manager/registered-courses/index.js"></script>
  ...
  <div class="manager grid grid-cols-5 gap-3 mt-[var(--height-header)] ">
    ...
    <div class="manager-container col-span-4 ">
      <div class="manager-content pt-3">
        <div class="text-[40px] tracking-wide font-bold font-sans mb-2">Danh sách đăng ký khóa học</div>
        <div class="group-content w-[calc(100%-12px)]">
          <div class="action mt-1 mb-6 flex">
            <div class="flex flex-1">
              <a href="./add.php" class="btn primary flex items-center"><i class="fa-solid fa-plus"></i> Thêm đăng ký khóa học</a>
            <form class="relative" action="./../../../handle/registered-course_handle.php" method="GET">
              <input type="hidden" name="action" value="search">
              <input name="s" value="<?php echo isset($_GET['s']) ? htmlspecialchars($_GET['s']) : ''; ?>" type="text" class="input-search" placeholder="Tìm kiếm đăng ký khóa học">
            ...
            <div class=""><p class="btn outline flex items-center !cursor-default"><?=$totalRegiateredCourse?> đăng ký</p></div>
          </div>
            ...
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
              <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                  <? if ($total == 0) { ?>
                    <tr>
                      <td colspan="5" class="text-center py-4 text-gray-500">Không tìm thấy đánh giá nào.</td>
                    </tr>
                  <? }; ?>
                  ...
                  ?>
                      <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="px-6 py-4">
                          <?= htmlspecialchars($registeredCourses[$i]['nameCourse']) ?>
                        </td>
                        <td class="px-6 py-4">
                          <?= htmlspecialchars($registeredCourses[$i]['username']) ?>
                        </td>
                        <td class="px-6 py-4">
                          <?= htmlspecialchars($registeredCourses[$i]['costed']) ?>
                        </td>
                        <td class="px-6 py-4 text-right action-object">
                          <a href="./edit.php?id=<?= $registeredCourses[$i]['idRegis'] ?>" class=""><i class="fa-solid fa-pen-to-square action-object-item"></i></a>
                          <!-- <a href="#" class=""><i class="fa-solid fa-circle-info action-object-item"></i></a> -->
                          <button
                            data-modal-target="popup-modal"
                            data-modal-toggle="popup-modal"
                            data-registered-course-id="<?= htmlspecialchars($registeredCourses[$i]['idRegis']) ?>"
