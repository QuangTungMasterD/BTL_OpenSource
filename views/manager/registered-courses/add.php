<?
  ...
  $backUrl = $_SESSION['prev_url'] ?? '/BTL-N2/views/manager/registered-courses/index.php';
...
<title>Thêm đánh đăng ký khóa học</title>
...
<div class="max-w-[600px] overflow-hidden mx-auto bg-white p-6 mt-12 rounded-xl mb-12">
  <h1 class="text-3xl mb-6 text-[rgb(var(--primary-color)] flex justify-center font-[450] uppercase">Thêm đánh đăng ký khóa học</h1>
  <form action="./../../../handle/registered-course_handle.php" method="POST">
    ...
    <div class="form-input">
      ...
      <select class="select" name="student" id="student">
        <? foreach($users as $r) { ?>
          <option value="<?= $r['idUser'] ?>" <?= ($old['student'] ?? '') == $r['idUser'] ? 'selected' : '' ?>>
    ...
    <div class="form-input">
      ...
      <input type="number" name="costed" class="input" placeholder="Nhập số tiền đăng ký" value="<?=htmlspecialchars($old['costed'] ?? 0) ?>" />
